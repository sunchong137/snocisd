...
save_file = f"data/hchain{bl}_{mol.basis}_tvecs_ndets{n_dets}.npy"
...
npoint = 100 #(xmax1-xmin1)*2

ds = np.linspace(xmin, xmax, npoint)
...
for i in range(lv):
    ...
    res.append(r)
res = np.asarray(res)
np.save(f"results/hchain{bl}_{mol.basis}_line.npy", res)
