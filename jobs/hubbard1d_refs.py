...
'''
import numpy as np
from noci_jax.misc import hubbard_ba, hubbard_dmrg
import os
...
def gen_ba():
    ...
    with open(res_dir + "/hubbard1d_ba.txt", "w") as fin:
        ...
        fin.write("# Bethe Ansatz ground state energies per site\n")
...
def gen_dmrg():
    ...
    with open(res_dir + "/hubbard1d_dmrg.txt", "w") as fin:
        fin.write("# 1D Hubbard model w PBC and half-filling\n")
        fin.write("# DMRG ground state energies per site\n")
        fin.write("# rows: number of sites, columns: U value, t = -1.\n")
        fin.write(" 0 ")
        for U in Uvals:
            fin.write("             {}".format(U))
        ...
        for n in nsites:
            ...
            fin.write("{:2d}".format(n))
            for U in Uvals:
                E = hubbard_dmrg.hubbard1d_dmrg(n, U, (ne_a, ne_a), pbc=True)
                fin.write("  {:2.10f}".format(E/n))
            fin.write("\n")
